...
namespace NaturalLanguageToolsUnitTests.Wikitext
{
    ...
    public class WikitextProcessorTests
    {
        ...
        public void ProcessorTest()
        {
            ...
            var storage = CreateStorage(text0, text1);

            var processed = processor.Transform(storage).First().Documents.Select(d => d.Data).ToArray();

            Assert.Equal(expected, processed);
        }

        private static IEnumerable<DocumentCollection<Tokens>> CreateStorage(params string[] texts)
        {
